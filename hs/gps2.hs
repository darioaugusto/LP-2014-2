...
appropriate_op goal operator | trace ("approp " ++ goal ++ " " ++ (op_action operator)) False = undefined
appropriate_op goal operator = member goal (op_add_list operator)
				
				
-- apply the operator and update the data				
apply_op :: [String] -> Op -> [String]
apply_op currents operator = if every currents (op_preconds operator) operator
							      then (setUnion (setDiff currents (op_del_list operator)) (op_add_list operator))
							      else currents

--Return true if true for all elements
every currents [] operator= True
every currents (precond:preconds) operator = (achieve precond currents [operator]) && (every currents preconds operator)

-- check if the goal is achievable from the current states and list of operators 
achieve :: String -> [String] -> [Op] -> Bool
achieve goal currents (operator:operators) | trace (concat currents) False = undefined
achieve goal currents (operator:operators) =  (if appropriate_op goal operator then ((if (currents == (apply_op currents operator)) then (member goal currents) else (achieve goal currents list_of_ops) )) else False) && (achieve goal currents operators)
achieve goal currents [] | trace (goal) False = undefined
achieve goal currents [] = (member goal currents) 

-- main call. Ex:  gps ["son_at_home","car_works"] ["son_at_school"] list_of_ops
gps currents [] ops = True				 
gps currents (goal:goals) ops = (achieve goal currents ops) && (gps currents goals ops)
